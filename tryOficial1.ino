...
void IgnoraCentrais() { //Inicio
  ...
  while (linhaEsq <= 2) {
    ...
    Serial.println(linhaEsq);
    if (LinhaDir() || LinhaTudo()) { //ha linha na direita
      Desloca(0, 130, 300);
      linhaEsq++;
    }
  }
...
void RotinaS() {
  ...
  Dir90();
  Dir90();
  while (!LinhaEsq() || !LinhaTudo()) { //Esquanto nao ha linha a equerda nem em todo o lado
    SeguirLinha();
  }
  Esq90();
